$.fn.distpickerSelf = function(province, city, areae) {
	var province_text = province + ' .model-select-text';
	var city_text = city + ' .model-select-text';
	var area_text = areae + ' .model-select-text';
	var province_lists = province + ' .model-select-option';
	var city_lists = city + ' .model-select-option';
	var area_lists = areae + ' .model-select-option';
	var province_item = province + ' ul li';
	var city_item = city + ' ul li';
	var area_item = areae + ' ul li';

	var pList = provinceList;
	var cList = provinceList[0].cityList;
	var aList = provinceList[0].cityList[0].areaList;
	
	$(province_text).text(pList[0].name);
	$(city_text).text(cList[0].name);
	$(area_text).text(aList[0]);

//	console.log($(province_text).text())
//	console.log($(city_text).text())
//	console.log($(area_text).text())

	//默认城市
	for(a = 0; a < pList.length; a++) {
		var pList_name = pList[a].name;
		var pList_li = "<li data-option=" + a + ">" + pList_name + "</li>";
		$(province).find('ul').append(pList_li);
	}
	for(b = 0; b < cList.length; b++) {
		var cList_name = cList[b].name;

		var cList_li = "<li data-option=" + b + ">" + cList_name + "</li>";
		$(city).find('ul').append(cList_li);
	}
	for(c = 0; c < aList.length; c++) {
		var aList_name = aList[c];
		var aList_li = "<li data-option=" + b + ">" + aList_name + "</li>";
		$(areae).find('ul').append(aList_li);
	}
	

	/*--------------点击省项-----------------*/
	$(province_item).click(function() {
		
		//-----------省---------------
		var province_item_text = $(this).text();
		$(province).find('.model-select-text').text(province_item_text);
		$(province).find('.model-select-text').attr('title', province_item_text);
		
		//获取当前点击的省的序号
		var province_index = $(this).index();
		
		//当前省所对应城市
		var province_city_l = provinceList[province_index].cityList;
		
		//默认第一个城市所对应的区县
		var province_area_l = provinceList[province_index].cityList[0].areaList;
		...
		//遍历城市列表给城市列表传值
		for(i = 0; i < province_city_l.length; i++) {
			//城市名
			var province_city_name = province_city_l[i].name;
			//城市列表项
			var province_city_li = "<li data-option=" + i + ">" + province_city_name + "</li>";
			//城市列表
			$(city).find('ul').append(province_city_li);
		}
			
		//当前省多对应第一个城市下面的区县
		var city_area_l= provinceList[province_index].cityList[0].areaList;
		...
		//遍历区县列表给区县列表传值
		for(j = 0; j < city_area_l.length; j++) {
			//区县名
			var city_area_l_name= city_area_l[j];
			//区县列表项
			var city_area_l_li= "<li data-option=" + j + ">" + city_area_l_name + "</li>";
			//区县列表
			$(areae).find('ul').append(city_area_l_li);
		}
		
		$(city_text).text(province_city_l[0].name);
		$(area_text).text(province_area_l[0]);
		
//		console.log(province_index)
		//当前省序号
		var $province_num=province_index;

		/*--------点击城市项----------*/
		$(city_item).click(function() {
			
			//-----------市--------------
			var city_item_text= $(this).text();
			//当前省序号
			var province_num= $province_num;
			//序号为空默认为第一项
			if(province_num == "") {
				province_num = 0;
			} else {
			...
			$(city).find('.model-select-text').text(city_item_text);
			$(city).find('.model-select-text').attr('title', city_item_text);
			
			//当前市序号
			var city_index= $(this).index();
//			console.log(city_index);
//			console.log(province_num);
			
			//当前市对应的区县列表
			var city_area_l= provinceList[province_num].cityList[city_index].areaList;
			...
			$(area_text).text(city_area_l[0]);
			
			var $city_num=city_index;
				
			$(area_item).click(function() {
				var area_item_text= $(this).text();
				$(areae).find('.model-select-text').text(area_item_text);
				$(areae).find('.model-select-text').attr('title', area_item_text);
	...
	$(city_item).click(function() {
		...
		for(j = 0; j < city_area_l.length; j++) {
			var city_area_l_name = city_area_ls[j];
			var city_area_l_li = "<li data-option=" + j + ">" + city_area_l_name + "</li>";
			$(areae).find('ul').append(city_area_l_li);
		}
		
		$(area_text).text(city_area_ls_name[0]);
		$city_num=city_index;
		/*------------------*/
		$(area_item).click(function() {
			var area_item_text = $(this).text();
	...
	function selectModel() {
		 ...
		 * 并隐藏页面中其他下拉列表
		 */
		$("div.model-select-text").click(function(e) {
			$('ul.model-select-option').not($(this).siblings('ul.model-select-option')).slideUp(speed, function() {
				int($(this));
			});
			$(this).siblings('ul.model-select-option').slideDown(speed, function() {
				int($(this));
			});
			return false;
		});
...
var provinceList = [{
	...
	cityList: [{
		name: '香港特别行政区',
		areaList: ['中西区','湾仔区','东区','南区','油尖旺区','深水埗区','九龙城区','黄大仙区','观塘区','荃湾区','屯门区','元朗区','北区','大埔区','西贡区','沙田区','葵青区','离岛区']
	}]
}, {
	name: '澳门特别行政区',
	cityList: [{
		name: '澳门特别行政区',
		areaList: ['花地玛堂区','花王堂区','望德堂区','大堂区','风顺堂区','嘉模堂区','路凼填海区','圣方济各堂区']
	}]
}];